...
const selectedTour = computed(() => {
  if (props.tourTitle && props.tourId) {
    const tour = TOURS_DATA.find((t) => t.id === Number(props.tourId));
    return `${props.tourTitle} - Duration: ${tour?.duration || ""} Days`;
  }
...
const tourDuration = computed(() => {
  if (props.tourId) {
    return TOURS_DATA.find((t) => t.id === Number(props.tourId))?.duration ?? null;
  }
  return null;
});
